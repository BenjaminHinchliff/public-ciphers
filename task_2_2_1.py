...
""",
    ...
    # pretend mallory did this somehow
    g = p
...
async def mallory(
    ...
    g = await alice_incoming.recv("g")
    await bob_outgoing.send(g, "g")

    await bob_outgoing.send(await alice_incoming.recv("A"), "A")
    await alice_outgoing.send(await bob_incoming.recv("B"), "B")

    k = SHA256.new((0).to_bytes(min_byte_length(0))).digest()
